...
class LeakyReLU:
    ...
    #Returns:
    # None, but once LeakyReLU layer is initialized, weights and biases are automatically
    # initialized and scaled.
    def __init__(self, indims, outdims, p=1):
        self.activation = (lambda z: np.maximum(0.01 * np.ones(z.shape) * z, z))
        self.derivative = (lambda z: np.where(z < 0, 0.01, 1))
